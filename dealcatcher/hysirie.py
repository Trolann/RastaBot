...
HYS_WEBSITE = 'https://hypeseeds.com/irie-genetics/'
...
thumbnail = 'https://hypeseeds.com/wp-content/uploads/2021/07/HypeSeeds-DM-2b-Final-01.png'
acronym = 'hys'
...
def get_all_hys_deals(website):
	...
	for page in range(1, num_pages + 1):
		...
		while(start_index != -1):
			deal_to_add = HYSDeal(html, start_index)
...
def process_hys_deals(old_hys_deals): # Take old list, get new list, produce print/expired/update the old list
	deals_to_print = []
	expired_deals = []

	active_hys_deals = get_all_hys_deals(HYS_WEBSITE)

	for new_deal in active_hys_deals:
		if new_deal not in old_hys_deals:
			deals_to_print.append(new_deal)

	for old_deal in old_hys_deals:
		if old_deal not in active_hys_deals:
			expired_deals.append(old_deal)

	old_hys_deals = active_hys_deals.copy()

	return (old_hys_deals, deals_to_print, expired_deals)

def hys_daemon(sleep_time):
	print('Hype Seeds Daemon started')
	...
	while True:
		...
		old_deals, new_deals, expired_deals = process_hys_deals(old_deals)
