...
HSB_WEBSITE = 'https://heritageseedbank.com/product-category/breeders/irie-genetics/'
page_str = 'page/{}'
item_start_str = 'product type-product' # Generates an item's start_index in Deal object
thumbnail = 'https://heritageseedbank.com/wp-content/uploads/2021/11/cropped-Resized_H-1.png'
acronym = 'hsb'
...
def get_all_hsb_deals(website):
	...
	for page in range(1, num_pages + 1):
		...
		while(start_index != -1):
			deal_to_add = HSBDeal(html, start_index)
...
def process_hsb_deals(old_hsb_deals): # Take old list, get new list, produce print/expired/update the old list
	deals_to_print = []
	expired_deals = []

	active_hsb_deals = get_all_hsb_deals(HSB_WEBSITE)

	for new_deal in active_hsb_deals:
		if new_deal not in old_hsb_deals:
			deals_to_print.append(new_deal)

	for old_deal in old_hsb_deals:
		if old_deal not in active_hsb_deals:
			expired_deals.append(old_deal)

	old_hsb_deals = active_hsb_deals.copy()

	return (old_hsb_deals, deals_to_print, expired_deals)

def hsb_daemon(sleep_time):
	print('Heritage Seed Bank Daemon started')
	...
	while True:
		...
		old_deals, new_deals, expired_deals = process_hsb_deals(old_deals)
