...
DSS_WEBSITE = 'https://www.dankseed.store/product-category/all-products/irie-genetics'
page_str = '/page/{}'  # Page not tested, assume 1 page
item_start_str = "<meta itemscope itemprop=\""  # Generates an item's start_index in Deal object
thumbnail = 'https://www.dankseed.store/wp-content/uploads/2020/11/mainlogo6-1.png'
acronym = 'dss'
...
class DSSDeal:
    ...
    def get_name(self, website_html, start_index):
        item = "Irie Genetics : "
        terminal = "\" />"
        name = website_html[website_html.find(item, start_index) +
                            len(item):website_html.find(
            terminal,
            website_html.find(item, start_index) +
            len(item))]
    ...
    def get_url(self, website_html, start_index):
        item = "itemid=\""
        terminal = "\" content="
        url = website_html[website_html.find(item, start_index) +
                           len(item):website_html.find(
            terminal,
            website_html.find(item, start_index) +
            len(item))]
    ...
    def get_amount(self, website_html, start_index):
        ...
        item = '&#36;</span>'
        terminal = "<"
        amount = website_html[website_html.find(item, start_index) +
                              len(item):website_html.find(
            terminal,
            website_html.find(item, start_index) +
            len(item))]
        ...
        if website_html.find("Sale!", start_index,
                             website_html.find("</li>", start_index)) > 0:
            start_index = website_html.find(
                item, start_index) + 1  # Start at the normal price
            amount = website_html[website_html.find(item, start_index) +
                                  len(item):website_html.find(
                terminal,
                website_html.find(item, start_index) +
    ...
    def get_image_url(self, website_html, start_index):
        ...
        url = website_html[website_html.find(item, start_index) +
            ...
            website_html.find(item, start_index) +
            len(item))]
        if url.find('image0') > 0:
            first_location = website_html.find(item, start_index) + len(item)
            url = website_html[website_html.find(item, first_location) +
                               len(item):website_html.find(
                terminal,
                website_html.find(item, first_location) +
                len(item))]
        return url
    ...
    def check_stock(self, website_html, start_index):
        ...
        stock_start_index = website_html.find("{}".format(item), start_index,
                                              end_index)
...
def get_all_dss_deals(website):
    ...
    for page in range(1, num_pages + 1):
        if page != 1:
            html = get_site('{}{}'.format(website,
                                          page_str.format(page)))

        start_index = html.find(item_start_str)
        while (start_index != -1):
            deal_to_add = DSSDeal(html, start_index)
...
def process_dss_deals(
        old_dss_deals
):  # Take old list, get new list, produce print/expired/update the old list
    deals_to_print = []
    expired_deals = []

    active_dss_deals = get_all_dss_deals(DSS_WEBSITE)

    for new_deal in active_dss_deals:
        if new_deal not in old_dss_deals:
            deals_to_print.append(new_deal)

    for old_deal in old_dss_deals:
        if old_deal not in active_dss_deals:
            expired_deals.append(old_deal)

    old_dss_deals = active_dss_deals.copy()

    return (old_dss_deals, deals_to_print, expired_deals)


def dss_daemon(sleep_time):
    print('Dank Seed Store Daemon started')
    ...
    while True:
        ...
        old_deals, new_deals, expired_deals = process_dss_deals(old_deals)
