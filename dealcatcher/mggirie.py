...
MGG_WEBSITE = 'https://mrgreengene.com/shop/product-category/hybrid/regular-seeds/irie-genetics'
page_str = '/page/{}'
item_start_str = 'product type-product' # Generates an item's start_index in Deal object
thumbnail = 'https://i0.wp.com/mrgreengene.com/shop/wp-content/uploads/2022/07/mrgreengene.com-logo-V2b-scaled.jpg?resize=1024%2C1024&ssl=1'
acronym = 'mgg'
...
def get_all_mgg_deals(website):
	...
	for page in range(1, num_pages + 1):
		...
		while(start_index != -1):
			deal_to_add = MGGDeal(html, start_index)
...
def process_mgg_deals(old_mgg_deals): # Take old list, get new list, produce print/expired/update the old list
	deals_to_print = []
	expired_deals = []

	active_mgg_deals = get_all_mgg_deals(MGG_WEBSITE)

	for new_deal in active_mgg_deals:
		if new_deal not in old_mgg_deals:
			deals_to_print.append(new_deal)

	for old_deal in old_mgg_deals:
		if old_deal not in active_mgg_deals:
			expired_deals.append(old_deal)

	old_mgg_deals = active_mgg_deals.copy()

	return (old_mgg_deals, deals_to_print, expired_deals)

def mgg_daemon(sleep_time):
	print('Mr. Green Gene Daemon started')
	...
	while True:
		...
		old_deals, new_deals, expired_deals = process_mgg_deals(old_deals)
